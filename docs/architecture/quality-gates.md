...
GitHub IssueからCodexが承認済みタスクを実装する場合も、必要なテストコードの追加・更新と、merge前に必要な品質観点は省略しません。ただし、対象リポジトリの既存GitHub Actionsが必要な品質ゲートを確実に実行する場合に限り、テストスイート、静的解析、型チェック、buildなどの実行をPull RequestのCIへ委譲できます。通常実行では、前節のローカル品質ゲートを維持します。

CIへ委譲する前に、workflowを正本として次を確認します。

- 対象のPull Requestとbase branchでworkflowが起動すること
- path filter、条件式、matrixなどにより、変更対象が必要なjobから除外されないこと
- 追加・更新したテストを含め、変更に必要なテスト、静的解析、型チェック、build、生成物チェックがjob内で実行されること

委譲する場合も、`git diff --check`、変更差分の確認、文書リンク、YAMLや設定ファイルの構文など、安全で軽量な検証は変更に応じてローカルで実行できます。タスクファイルとPull Request本文には、ローカルで実行した検証と結果、CIへ委譲した検証、根拠としたworkflow、CI結果が未確定であることを区別して記録します。未実行の検証や未確定のCI結果を成功として記録しません。

必要な品質ゲートを実行するCIが存在しない、または既存CIの対象範囲が不足するリポジトリでは、Issue駆動実行によるコードや設定の変更を開始しません。必要な検証方法をユーザーへ確認するか、CI整備を先行タスクとして分離します。文書だけの変更は、必要な軽量検証と残るリスクをタスクファイルとPull Requestに明示できる場合に限り進められます。

draft Pull Requestの作成後、CodexはCI結果を同じ実行内でポーリングし続けません。CIが失敗した場合は承認範囲内の修正を新しいタスクやPull Requestへ分けず、同じタスク、task branch、Pull Requestで行い、実行結果の記録も更新します。Pull RequestはCI結果とレビューを確認してからmergeし、自動mergeは行いません。
